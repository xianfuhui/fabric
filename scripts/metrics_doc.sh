...
generate_doc() {
    local gendoc_command="go run github.com/xianfuhui/fabric/common/metrics/cmd/gendoc"
    local orderer_prom
    ...
    local orderer_deps=()
    while IFS= read -r pkg; do orderer_deps+=("$pkg"); done < <(go list -deps github.com/xianfuhui/fabric/orderer | sort -u | grep hyperledger)
    ...
    local peer_deps=()
    while IFS= read -r pkg; do peer_deps+=("$pkg"); done < <(go list -deps github.com/xianfuhui/fabric/peer | sort -u | grep hyperledger)
